...
class TDABaselineAndFilterProxy:
    def _patch_to_region(self, patch):
        ...
        # Propaga image_path, vertebra_idx, centroid_x/y, split si existen
        return RegionRecord(
    ...
    def _run_tda_for_patches(self, patches, label, curve):
        ...
        region_records = []
        for p in patches:
            region = self._patch_to_region(p)
        ...
        outdir = os.path.join(self.patient_dir, f"pre_tda_{label}")
        ...
        # 6. (Opcional) Llamar TDA posterior si se requiere
